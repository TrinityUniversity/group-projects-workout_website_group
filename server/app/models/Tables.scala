...
trait Tables {
  ...
  case class WorkoutsRow(id: Int, userId: Int, videoUrl: String, sweatLevel: Int, intensity: Int, workLength: Int, workoutType: String, likes: Int, views: Int)
  ...
  implicit def GetResultWorkoutsRow(implicit e0: GR[Int], e1: GR[String]): GR[WorkoutsRow] = GR{
    prs => import prs._
    WorkoutsRow.tupled((<<[Int], <<[Int], <<[String], <<[Int], <<[Int], <<[Int], <<[String], <<[Int], <<[Int]))
  }
  ...
  class Workouts(_tableTag: Tag) extends profile.api.Table[WorkoutsRow](_tableTag, "workouts") {
    def * = (id, userId, videoUrl, sweatLevel, intensity, workLength, workoutType, likes, views).<>(WorkoutsRow.tupled, WorkoutsRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(userId), Rep.Some(videoUrl), Rep.Some(sweatLevel), Rep.Some(intensity), Rep.Some(workLength), Rep.Some(workoutType), Rep.Some(likes), Rep.Some(views))).shaped.<>({r=>import r._; _1.map(_=> WorkoutsRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get, _8.get, _9.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Foreign key referencing Users (database name workouts_user_id_fkey) */
    lazy val usersFk = foreignKey("workouts_user_id_fkey", userId, Users)(r => r.userId, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
  }
